...
Respond in ${langName}. 2-3 paragraphs. Absolute confidentiality about the other party's specific words.`;
  ...
  const callSergio=async(userMsg, histOverride)=>{
    ...
    try {
      ...
      const donePhrasesA=["retornarei a você","retornarei para","retornaremos","I will return","volveré","je reviendrai","tornerò","戻ります","我将回来"];
  ...
  useEffect(()=>{
    ...
    if(s){
      ...
      store.get(`session:${code}`).then(data=>{
        setSessionCode(code); setSessionData(data);
